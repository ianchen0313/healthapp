...
with st.expander("查看更多關於模型表現"):
    st.text('原始資料經整理後共有 11283 個有效樣本，其中患有腎臟病者為 494 人，佔有效樣本數 4%')
    st.text('團隊將資料進行 8:2 切割分成訓練集與驗證集，以下數據為模型訓練完成後在驗證集的表現')
    st.text('（採 bootstrap 法建構之 95% 信賴區間）')
    st.text('Accuracy(整體預測正確率): [0.7356, 0.7398]')
    st.text('Sensitivity(實際有病者中預測為有病比率): [0.5756, 0.6267]')
    st.text('Specificity(實際沒病者中預測為沒病比率): [0.7420, 0.7461]')
